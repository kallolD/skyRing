...
C_SI =  jnp.float64(lal.C_SI) #299_792_458.0  # m/s
...
def _src_unit_ecef(ra, dec, gmst):
    ...
    return jnp.stack([cosd * jnp.cos(H),   # x (lon=0, lat=0)
                      cosd * jnp.sin(H),   # y (lon=90E, lat=0)
                      jnp.sin(dec)], axis=-1)

# @jax.jit
def dt_L1_minus_H1(ra, dec, gmst, rL_ecef, rH_ecef):
    """
    Returns t_L1 - t_H1 in seconds.
    ra,dec can be scalars or arrays; broadcasting works.
    gmst scalar (radians).
    rL_ecef, rH_ecef: shape (3,) in meters (ECEF).
    """
    s_hat = _src_unit_ecef(ra, dec, gmst)        # (..., 3)
    baseline = rL_ecef - rH_ecef                 # (3,)
    # t = t_geo - (r · s_hat)/c  =>  dt = -(baseline · s_hat)/c
    return -jnp.sum(baseline * s_hat, axis=-1) / C_SI

def set_detectors(resp_H, resp_L):
    """
    initialize the detector responses.
...
def set_detector_locations(rH, rL):
    """
    initialize the detector locations.
    rH, rL: (3,1) JAX arrays 
    """
    global _rH, _rL
    _rH = rH
    _rL = rL
...
def simulate_data(params, psd1, psd2, L1, L2, omega_r, omega_i, omega_OT_r, omega_OT_i,
    ...
    for j in range(A.shape[0]):
        waveform1 = waveform1 + wvf_funcs[j](t+t0, f[j], gamma[j], A[j], Fp1, Fc1, swsh22p, swsh22m)
        waveform2 = waveform2 + wvf_funcs[j](t+t0, f[j], gamma[j], A[j], Fp2, Fc2, swsh22p, swsh22m)
...
def ln_likelihood(data1, data2, params, L1, L2, omega_22_r, omega_22_i, omega_33_r, omega_33_i,
                     Fp1, Fc1, Fp2, Fc2, swsh22p, swsh22m, swsh33p, swsh33m, T, srate, t0, t2_minus_t1, ref_det):

    M, chi, A220, P220, A330, P330 = jnp.asarray(params,dtype=jnp.float64)
    prefactor_omega_r = (1./(2.*jnp.pi*M*MTSUN))
    prefactor_omega_i = (1./(M*MTSUN))

    # Mode amplitude ratio condition
    valid = A330<A220

    A220 = A220*jnp.exp(1j*P220)*reference_amplitude
    A330 = A330*jnp.exp(1j*P330)*reference_amplitude
    A = jnp.array([A220, A330],dtype=jnp.complex128)

    f = jnp.array([prefactor_omega_r*omega_22_r(chi), prefactor_omega_r*omega_33_r(chi)],dtype=jnp.float64)
    gamma = jnp.array([-prefactor_omega_i*omega_22_i(chi), -prefactor_omega_i*omega_33_i(chi)],dtype=jnp.float64)
    ...
    wvf_funcs = [h22, h33]
    swshs_p = [swsh22p, swsh33p]
    swshs_m = [swsh22m, swsh33m]
    waveform1 = jnp.zeros(N, dtype=jnp.float64)
    waveform2 = jnp.zeros(N, dtype=jnp.float64)

    if ref_det=='H1':
        for j in range(A.shape[0]):
            waveform1 = waveform1 + wvf_funcs[j](t+t0, f[j], gamma[j], A[j], Fp1, Fc1, swshs_p[j], swshs_m[j])
            waveform2 = waveform2 + wvf_funcs[j](t+t0-t2_minus_t1, f[j], gamma[j], A[j], Fp2, Fc2, swshs_p[j], swshs_m[j]) #amplitude also compensated
    elif ref_det=='L1':
        for j in range(A.shape[0]):
            waveform1 = waveform1 + wvf_funcs[j](t+t0+t2_minus_t1, f[j], gamma[j], A[j], Fp1, Fc1, swshs_p[j], swshs_m[j])
            waveform2 = waveform2 + wvf_funcs[j](t+t0, f[j], gamma[j], A[j], Fp2, Fc2, swshs_p[j], swshs_m[j]) #amplitude also compensated
    else:
    ...
    logl = -0.5*(jnp.vdot(y1, y1).real+jnp.vdot(y2, y2).real)
    
    logl = jnp.where(valid, logl, -jnp.inf)
    return logl


def ln_likelihood_full(dataH, dataL, params, gmst, L_H, L_L, omega_22_r, omega_22_i, omega_33_r, omega_33_i, T, srate, t0, ref_det='H1'): 
    ...
    return ln_likelihood(dataH, dataL, p0, L_H, L_L, omega_22_r, omega_22_i, omega_33_r, omega_33_i,
            FpH, FcH, FpL, FcL, swsh22p, swsh22m, swsh33p, swsh33m, T, srate, t0, t2_minus_t1, ref_det)
